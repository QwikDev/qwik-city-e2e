...
const getClientDataPath = (pathname, pageSearch, action) => {
  ...
  if (action)
    search +=
      (search ? "&" : "?") + QACTION_KEY + "=" + encodeURIComponent(action.id);
  return (
    pathname + (pathname.endsWith("/") ? "" : "/") + "q-data.json" + search
  );
};
...
const getPrefetchDataset = (props, clientNavPath, currentLoc) => {
  if (props.prefetch === true && clientNavPath) {
    ...
    if (!isSamePathname(prefetchUrl, toUrl("", currentLoc))) return "";
  }
  return null;
};
...
const loadClientData = async (href, clearCache, action) => {
  ...
  if (!qData) {
    ...
    qData = fetch(clientDataPath, options).then((rsp) => {
      ...
      if (
        redirectedURL.origin !== location.origin ||
        !isQDataJson(redirectedURL.pathname)
      ) {
      ...
      if ((rsp.headers.get("content-type") || "").includes("json"))
        return rsp.text().then((text) => {
          ...
          if (clientData.__brand !== "qdata") return;
      ...
      else CLIENT_DATA_CACHE.delete(clientDataPath);
    });
...
const QwikCityProvider = /* @__PURE__ */ componentQrl(
  inlinedQrl(() => {
    ...
    const currentAction = currentActionId
      ? env.response.loaders[currentActionId]
      : void 0;
      ...
      inlinedQrl(
        ({ track }) => {
          ...
          async function run() {
            ...
            const { routes, menus, cacheModules, trailingSlash } = globalThis[
              "something"
            ]
              ? await import("@qwik-city-plan")
              : {};
            console.log("run2");
            ...
            let clientPageData;
            if (isServer) clientPageData = env2.response;
            else {
            ...
            if (url2.pathname.endsWith("/")) {
              if (!trailingSlash) url2.pathname = url2.pathname.slice(0, -1);
            } else if (trailingSlash) url2.pathname += "/";
      ...
      "a",
      {
        ...
        onClick$: inlinedQrl(
          () => {
            ...
            if (clientNavPath2) nav2(linkProps2.href);
          },
        ...
        onMouseOver$: inlinedQrl(
          ...
          "Link_component_a_onMouseOver_yiXwCC0m3jY"
        ),
        onFocus$: inlinedQrl(
          (_, elm) => prefetchLinkResources(elm),
          "Link_component_a_onFocus_PrXIxv2vNXY"
        ),
        onQVisible$: inlinedQrl(
          (_, elm) => prefetchLinkResources(elm, true),
          "Link_component_a_onQVisible_EpaZ5qQ4Lg4"
        ),
...
class ServerActionImpl {
  ...
  use() {
    ...
    initialState.run = inlinedQrl(
      ...
      [currentAction, initialState, loc, state]
    );
    return state;
  }
}
...
const Form = ({ action, spaReset, reloadDocument, onSubmit$, ...rest }) => {
  return jsx("form", {
    ...
    onSubmit$: [!reloadDocument ? action.run : void 0, onSubmit$],
    method: "post",
